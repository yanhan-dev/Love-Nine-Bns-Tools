...
namespace LoveNineBnsTools
{
    ...
    public class BNSDat
    {
        ...
        public string AES_KEY = "jan_2#0_cpl_bns!";
        ...
        public void Extract(string FileName, bool is64 = false, Func<string, string> dispalyProgress = null)
        {
            ...
            for (int i = 0; i < FileCount; i++)
            {
                ...
                if (file_path.EndsWith("xml") || file_path.EndsWith("x16"))
                {
                    ...
                    Converts(temp2, bns_xml.DetectType(temp2), temp, BXML_TYPE.BXML_PLAIN);
                    temp2.Close();
                ...
                // Report progress
                //dispalyProgress(Convert.ToInt32((i+1 / (FileCount * 1.0)) * 100));
                dispalyProgress?.Invoke(string.Format("正在解包: {0}/{1}", i+1, FileCount));
                // 解包状态
                // End report progress
            }

            // Report job done
            dispalyProgress?.Invoke("解包完成！");
            // 解包完成
            // End report
        ...
        public void Compress(string Folder, bool is64 = false, Func<string, string> dispalyProgress = null, int compression = 9)
        {
            string file_path;
            byte[] buffer_packed;
            byte[] buffer_unpacked;
            ...
            for (int i = 0; i < FileCount; i++)
            {
                ...
                if (file_path.EndsWith(".xml") || file_path.EndsWith(".x16"))
                {
                    ...
                    Converts(fis, bxml.DetectType(fis), tmp, BXML_TYPE.BXML_BINARY);
                }
                else
                {
                    // compress raw
                ...
                // Report progress
                dispalyProgress?.Invoke(string.Format("正在打包: {0}/{1}", i + 1, FileCount));
                // 打包状态
                // End report progress
            }

            // Report job done
            dispalyProgress?.Invoke("打包完成！");
            // 打包完成,正在写出文件...
            // End report
